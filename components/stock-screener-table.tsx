...
import { RiArrowUpDownLine, RiCheckboxCircleFill } from '@remixicon/react';
            ...
            <div className="p-2 text-sm text-muted-foreground bg-muted border-t">
              {`${
                data[0].period.toLowerCase() === 'ttm'
                  ? 'TTM'
                  : data[0].period.charAt(0).toUpperCase() +
                    data[0].period.slice(1)
              } data in ${data[0].currency}`}
            </div>
          </AccordionContent>
        </AccordionItem>
      </Accordion>
